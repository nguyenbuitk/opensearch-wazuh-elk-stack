...
*/}}
{{- define "wazuh.dockerconfigjson" -}}
{
    "auths": {
        "{{ .Values.image.credentials.registry }}": {
            "auth": {{ printf "%s:%s" .Values.image.credentials.username .Values.image.credentials.password | b64enc | quote }}
        }
    }
}
{{- end }}
...
{{- $replicas := .Values.replicaCount.elasticsearch | int }}
  {{- range $i, $e := untilStep 0 $replicas 1 -}}
    {{ template "wazuh.fullname" $ }}-elasticsearch-{{ $i }},
  {{- end -}}
{{- end -}}
